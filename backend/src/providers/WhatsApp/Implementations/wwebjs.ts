...
const editMessage = async (
  ...
  const result = await message.edit(newBody);
  if (result === null) {
...
const init = async (whatsapp: Whatsapp): Promise<void> => {
  try {
    ...
    wbot.on("message_edit", async (msg, newBody, oldBody) => {
      try {
        await handleMessageEdit(msg.id.id, newBody as string, oldBody as string);
      } catch (err) {
        logger.error(err, "Error on whatsapp message edit event");
      }
    });
